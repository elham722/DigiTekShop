    ...
    where T : AggregateRoot<TId>
{
    ...
    public EfQueryRepository(DigiTekShopDbContext ctx)
    {
        _ctx = ctx;
        _set = ctx.Set<T>();
    }
    ...
    {
        IQueryable<T> q = _set.AsNoTracking().Where(predicate);
        if (includes != null)
            foreach (var inc in includes)
                q = q.Include(inc);
        return await q.ToListAsync(ct);
    ...
    public Task<bool> ExistsAsync(TId id, CancellationToken ct = default)
        => _set.AsNoTracking().AnyAsync(e => e.Id!.Equals(id), ct);
    ...
    public IQueryBuilder<T, TId> CreateQuery()
        => new EfQueryBuilder<T, TId>(_set.AsNoTracking());
}
